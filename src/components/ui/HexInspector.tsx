  ...
  return (
    ...
    >
      ...
      <div className="space-y-2 overflow-y-auto no-scrollbar">
        ...
        <InspectorRow label="Signed Int16 (BE)" value={getInt16().toString()} />
        <InspectorRow label="Unsigned Int16 (BE)" value={getUint16().toString()} />
        <InspectorRow label="Signed Int32 (BE)" value={getInt32().toString()} />
        <InspectorRow label="Float32 (BE)" value={getFloat32().toFixed(6)} />
...
const InspectorRow = ({ label, value }: { label: string, value: string }) => (
  <div className="flex justify-between items-center p-2 bg-black/20 rounded border border-white/5 group hover:border-cyan-500/30 transition-all">
    <span className="text-[9px] text-gray-500 font-bold group-hover:text-gray-300 transition-colors">{label}</span>
    <span className="text-[11px] text-cyan-300 font-mono select-all">{value}</span>
  </div>
);
